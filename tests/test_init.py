...
from homecom_alt import HomeComAlt, ConnectionOptions, ApiError, AuthFailedError, NotRespondingError
import jwt
...
def create_test_jwt(expiration: int = 9999999999):
...
@pytest.mark.asyncio
async def test_async_http_request_success_json():
    session = ClientSession()
    options = ConnectionOptions(username="test_user", password="test_pass", token=create_test_jwt(), refresh_token="test_refresh") 

    bhc = await HomeComAlt.create(session, options)
    with (
        aioresponses() as session_mock,
        patch(
            "homecom_alt.HomeComAlt._async_http_request"
        ) as mock_request,
    ):
    ...
    assert mock_request.call_args[0][1] == f"http://test.com"

@pytest.mark.asyncio
async def test_async_http_request_success_form():
    session = ClientSession()
    options = ConnectionOptions(username="test_user", password="test_pass", token=create_test_jwt(), refresh_token="test_refresh") 
...
@pytest.mark.asyncio
async def test_async_http_request_unauthorized():
    session = ClientSession()
    options = ConnectionOptions(username="test_user", password="test_pass", token=create_test_jwt(), refresh_token="test_refresh") 
...
@pytest.mark.asyncio
async def test_async_http_request_bad_request():
    session = ClientSession()
    options = ConnectionOptions(username="test_user", password="test_pass", token=create_test_jwt(), refresh_token="test_refresh") 
    ...
    with patch.object(ClientSession, 'request', new=AsyncMock()) as mock_request:
        ...
        response = await bhc._async_http_request("post", "https://singlekey-id.com/auth/connect/token")
        assert response is None

@pytest.mark.asyncio
async def test_async_http_request_timeout():
    session = ClientSession()
    options = ConnectionOptions(username="test_user", password="test_pass", token=create_test_jwt(), refresh_token="test_refresh") 
...
@pytest.mark.asyncio
async def test_async_http_request_invalid_status():
    session = ClientSession()
    options = ConnectionOptions(username="test_user", password="test_pass", token=create_test_jwt(), refresh_token="test_refresh") 
...
@pytest.mark.asyncio
async def test_check_jwt_valid():
    session = ClientSession()
    options = ConnectionOptions(token=create_test_jwt(), refresh_token="test_refresh_token", username="test_user", password="test_pass")
    homecom_alt: HomeComAlt = await HomeComAlt.create(session, options)
    assert homecom_alt.check_jwt() is True

@pytest.mark.asyncio
async def test_check_jwt_invalid():
    session = ClientSession()
    options = ConnectionOptions(token=create_test_jwt(), refresh_token="test_refresh_token", username="test_user", password="test_pass")
    homecom_alt: HomeComAlt = await HomeComAlt.create(session, options)
    homecom_alt._options.token = create_test_jwt(expiration=int((datetime.now(UTC) - timedelta(days=1)).timestamp()))
    assert homecom_alt.check_jwt() is False

@pytest.mark.asyncio
async def test_get_token_valid_jwt():
    session = ClientSession()
    options = ConnectionOptions(token=create_test_jwt(), refresh_token="test_refresh_token", username="test_user", password="test_pass")
    homecom_alt: HomeComAlt = await HomeComAlt.create(session, options)
...
@pytest.mark.asyncio
async def test_do_auth_step3_success():
...
@pytest.mark.asyncio
async def test_do_auth_step3_unauthorized():
    """Test do_auth_step3 raising AuthFailedError on 401."""
    session = ClientSession()
    homecom = HomeComAlt(session, ConnectionOptions(password="test_pass"))

    with patch.object(session, "post", new=AsyncMock()) as mock_post:
        mock_post.side_effect = ClientResponseError(None, (), status=HTTPStatus.UNAUTHORIZED)
...
@pytest.mark.asyncio
async def test_do_auth_step_failure():
    """Test do_auth failing when a step fails."""
    session = ClientSession()
    homecom = HomeComAlt(session, ConnectionOptions())

    with patch.object(homecom, "do_auth_step1", new=AsyncMock(side_effect=AuthFailedError("Step 1 failed"))):
        with pytest.raises(AuthFailedError, match="Step 1 failed"):
...
@pytest.mark.asyncio
async def test_validate_auth_invalid_response():
