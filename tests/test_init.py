...
def _k40_bulk_route(url, args, kwargs, overrides=None):  # noqa: ANN001, ANN202, ARG001
    """Handle a K40 bulk request in a mock route function.

    *overrides* is an optional dict mapping path substrings to payloads.
    """
    posted_data = args[0] if args else kwargs.get("data", [{}])
    resource_paths = posted_data[0].get("resourcePaths", [])
    extra = overrides or {}

    def _resolve(path):  # noqa: ANN001, ANN202, PLR0911, PLR0912
        # Check overrides first
        for needle, payload in extra.items():
            if needle in path:
                return payload
        # Default responses for standard K40 bulk endpoints
    ...
    async def route(method, url, *args, **kwargs):  # noqa: ANN001, ANN002, ANN003, ANN202, ARG001, PLR0911, PLR0912
        if "bulk" in url:
            return _k40_bulk_route(
                ...
                kwargs,
                overrides={
                    "/zones": {"references": [{"id": "/zones/zn1"}]},
                    "/devices": {"references": [{"id": "/devices/device1"}]},
...
async def test_commodule_cp_start_charging() -> None:
    ...
    with patch.object(
        ...
        await cm.async_cp_start_charging(DEVICE_ID, "cp0", "MyWallbox")
        assert mock_req.call_count == 1
        assert "/cmd/start" in mock_req.call_args_list[0][0][1]
        assert mock_req.call_args_list[0][0][2] == {"name": "MyWallbox"}
...
async def test_commodule_cp_pause_charging() -> None:
    ...
    with patch.object(
        ...
        await cm.async_cp_pause_charging(DEVICE_ID, "cp0", "MyWallbox")
        assert mock_req.call_count == 1
        assert "/cmd/pause" in mock_req.call_args_list[0][0][1]
        assert mock_req.call_args_list[0][0][2] == {"name": "MyWallbox"}
...
@pytest.mark.asyncio
async def test_commodule_cp_set_limit() -> None:
    """Test set charging limit command."""
    session = ClientSession()
    cm = _make_commodule(session)
    ...
    with patch.object(
        ...
        await cm.async_cp_set_limit(DEVICE_ID, "cp0", 16)
        assert mock_req.call_count == 1
        assert "/cmd/limit" in mock_req.call_args_list[0][0][1]
        assert mock_req.call_args_list[0][0][2] == {"limit": 16}
